...
class _RootShellState extends State<RootShell> with WidgetsBindingObserver {
  ...
  Future<void> selectLocation(
    ...
    // A cached GPS fix can resolve before the launch load's `finally` clears
    // its loading flag. Wait it out so the location-scoped reload is not
    // discarded by loadAppData's concurrent-request guard.
    while (mounted && isLoadingAppData) {
      await Future<void>.delayed(const Duration(milliseconds: 50));
    }
    if (mounted) await loadAppData();
  }
