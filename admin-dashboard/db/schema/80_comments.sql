...
COMMENT ON TABLE public.order_handoffs IS 'One row per completed pickup handoff: who released the order, at which cart, and whether the customer proved it with a scanned QR, a typed six-digit code, or a staff override. Append-only; the override rate is the health metric for the pickup process.';
COMMENT ON COLUMN public.order_handoffs.method IS 'How the customer was proven: qr (scanned), short_code (six digits typed from the same token), override (no code — requires reason_code).';
COMMENT ON COLUMN public.order_handoffs.staff_label IS 'Name of the attendant at handoff time. Snapshot, so it survives employee_id being nulled, and covers ADMIN_USERS logins that have no employees row.';
COMMENT ON COLUMN public.order_handoffs.token_nonce IS 'Nonce of the pickup token spent on this handoff. Uniquely indexed, so a captured code cannot be replayed. Null only for overrides, which have no token.';
