-- ============================================================================
-- EBTL — Supabase schema backup: triggers
-- Requires 05_functions.sql (public + private trigger functions).
-- ============================================================================
...
CREATE TRIGGER trg_customer_payment_methods_updated_at BEFORE UPDATE ON public.customer_payment_methods FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
CREATE TRIGGER trg_customer_push_tokens_updated_at BEFORE UPDATE ON public.customer_push_tokens FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
CREATE TRIGGER trg_order_prep_tasks_updated_at BEFORE UPDATE ON public.order_prep_tasks FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();

-- orders: order matters (order_number/business_date, then confirmed_at, then status stamps)
CREATE TRIGGER set_order_confirmed_at BEFORE INSERT OR UPDATE OF status, payment_status, confirmed_at ON public.orders FOR EACH ROW EXECUTE FUNCTION public.set_order_confirmed_at();
CREATE TRIGGER trg_consume_inventory_when_order_completed AFTER UPDATE OF status ON public.orders FOR EACH ROW EXECUTE FUNCTION public.consume_inventory_when_order_completed();
...
CREATE TRIGGER trg_stamp_order_status_timestamps BEFORE UPDATE ON public.orders FOR EACH ROW EXECUTE FUNCTION public.stamp_order_status_timestamps();

CREATE TRIGGER trg_payments_updated_at BEFORE UPDATE ON public.payments FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
CREATE TRIGGER trg_prep_stations_updated_at BEFORE UPDATE ON public.prep_stations FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
...
CREATE TRIGGER trg_product_tags_updated_at BEFORE UPDATE ON public.product_tags FOR EACH ROW EXECUTE FUNCTION public.set_updated_at();
