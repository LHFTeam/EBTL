-- ============================================================================
-- EBTL — Supabase schema backup: grants, realtime publication, comments
-- ============================================================================
...
-- ---------------------------------------------------------------------------
-- Grants
-- Baseline matches the Supabase default for schema public, then the four
-- server-only tables have anon/authenticated revoked (service_role only).
-- ---------------------------------------------------------------------------
...
-- Server-only tables: no direct client access at all.
REVOKE ALL ON public.customer_credit_ledger FROM anon, authenticated;
...
-- The private schema is not exposed to PostgREST roles.
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_publication_rel pr
        JOIN pg_publication p ON p.oid = pr.prpubid
        WHERE p.pubname = 'supabase_realtime' AND pr.prrelid = 'public.order_items'::regclass
    ) THEN
...
COMMENT ON COLUMN public.customers.apple_user_id IS 'Apple subject ID from social sign-in. Null for customers who never signed in.';
COMMENT ON COLUMN public.customers.facebook_user_id IS 'Facebook subject ID from social sign-in. Null for customers who never signed in.';
COMMENT ON COLUMN public.customers.google_user_id IS 'Google subject ID from social sign-in. Null for customers who never signed in.';
